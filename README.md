...
Тестирование является критически важным этапом разработки.

### Типы тестирования:

1. Юнит-тесты: Изолированные тесты компонентов.
2. Интеграционные тесты: Проверка взаимодействия между компонентами системы.
3. End-to-end (E2E) тесты: Полное тестирование системы в условиях, приближенных к реальным.

### Почему важно тестировать:

1. Надежность кода: TypeScript не гарантирует полную безопасность типов на этапе выполнения и не проверяет бизнес-логику.
2. Эффективность: Автоматические тесты экономят время по сравнению с ручным тестированием.
3. Необходимость для бэкенда и фронтенда: Критически важно для бэкенда, важно для удобства в фронтенде.

### Подходы к тестированию:

1. E2E тесты: Идеально подходят для начала тестирования API, позволяют проверить основные запросы и ответы.
2. Юнит-тесты: Лучший выбор для сложной бизнес-логики, выполняются без необходимости полного окружения.
3. Интеграционные тесты: Полезны для проверки взаимодействия между несколькими компонентами или сервисами.

### Инструментарий:

1. Jest: Основной инструмент для бэкенда с поддержкой мок-функций и необходимых для проверки инструментов.
2. Для фронтенда: Cypress, Paratractor, Puppeteer для E2E тестирования и Jest, Mock, Chai для юнит-тестирования.
3. API тестирование: Используются инструменты вроде Супертеста, Axios, Fetch для интеграции и проверки API.

## 9.2. Unit тесты

Процесс написания юнит-тестов для изолированного тестирования части бизнес-логики приложения.

Юнит-тесты позволяют тестировать отдельные компоненты приложения без его полного запуска.
Dependency Injection (DI) и Inversion of Control (IoC) облегчают этот процесс.

Шаги

1. Настройка окружения для тестирования:
   Установка `Jest` и настройка зависимостей через NPM.
   Создание конфигурационного файла `Jest`.

```shell
npm i -D jest @types/jest ts-jest
```

Сообщение при установке об устаревших пакетах
...
`ts-jest` аналогичен `ts-node` он позволяет запускать файлы TypeScript

Пример конфигурационного файла `jest.config.ts`:

```TypeScript
import type { Config } from '@jest/types';

const config: Config.InitialOptions = {
	verbose: true, // Показывать детальный Output
	preset: 'ts-jest',
};
...
2. Подготовка к написанию теста:
   Размещение тестовых файлов рядом с тестируемыми компонентами. E2E тесты обычно находятся в отдельных папках, поскольку они тестируют все приложение.

   Расширение файлов-тестов обычно `.spec.ts`, например: `users\users.service.spec.ts`. В этом случае Jest будет подтягивать все необходимые файлы `.spec` из каталога `src`.

   Использование хуков `beforeAll` и `afterAll` для настройки и очистки перед и после тестов. Это необходимо чтобы собрать частично сервис не запуская все приложение.

   `beforeAll` - функция, которая будет выполняться перед всеми тестами.

   `beforeEach` - функция, которая будет выполняться перед каждым тестом.

3. Структура теста:
   Структура включает `describe` для группировки тестов, `it` для определения конкретных тестов.
   Асинхронное выполнение с использованием `async/await`.

describe — это группа тестов, логически объединяющая несколько it-тестов.

`describe(name, fn)`

📛 Почему называется describe?
Название пошло от описательного стиля тестирования (`BDD` — `Behavior-Driven Development`).

`describe` переводится как «опиши». То есть буквально описывается поведение или компонент, который тестируется.

Пример (читается как: «Опиши, как работает User Service»):
...
it — это один конкретный тест, проверяющий определённое поведение.

`it(name, fn)`

📛 Почему называется it?

it — это начало предложения, которое читается как:
It should do something — «Оно должно…»
...
Читается как: «Оно должно создать пользователя»
(или: It should create a user — если бы ты писал полное предложение).

Такой стиль помогает читать тесты как документацию:
...
«UserService должен создавать пользователя с хешированным паролем».

Можно писать и более естественно:

```ts
it('returns null if user not found');
```

4. Написание юнит-теста:
   Имитация зависимостей с помощью моков.
   Замена реальных компонентов на моки в контейнере зависимостей.
   Проверка результатов действий сервиса.

Пример теста `users\users.service.spec.ts`:
...
5. Запуск и проверка тестов:
   Добавление команды для запуска тестов в `package.json`.
...
Валидация успешного прохождения тестов.
...
> jest
 ...
 PASS  dist/src/users/users.service.spec.js
  User Service
    √ createUser (27 ms)
...
Snapshots:   0 total
Time:        2.743 s, estimated 3 s
Ran all test suites.
```

### Ключевые Моменты:

Юнит-тестирование эффективно благодаря модульной структуре приложения и `DI`.
`Jest` предоставляет удобные инструменты для имитации зависимостей и проверки ожидаемых результатов.
Разделение тестируемого кода от его зависимостей позволяет точечно проверять его функциональность.

## 9.3. Unit тесты функции validateUser

Тестирование функцию validateUser. Напишем для этого три различных теста.

Подготовка:

- Используем созданный ранее `createdUser` для упрощения процесса.
- Обязательно замокаем все зависимости функции.

Лучше не использовать результаты предыдущих тестов, для выполнения последующих. Все тесты должны быть максимально независимы.
Но в данном примере для упрощения используется `createdUser` созданный в предыдущем тесте.

### Тестовые сценарии:

1. Успешная валидация пользователя:
   Мокаем возврат функции `userRepository.find` чтобы она возвращала `createdUser`.
   Вызываем `userService.validateUser` с корректным email и паролем.
   Ожидаем, что результат вызова - `true`.

2. Валидация с неверным паролем:
   Используем тот же мок `userRepository.find` для возврата `createdUser`.
   Вызываем `userService.validateUser` с неверным паролем.
   Ожидаем, что результат вызова - `false`.

3. Валидация несуществующего пользователя:
   Мокаем `userRepository.find` чтобы она возвращала `null`.
   Вызываем `userService.validateUser` с любыми данными.
   Ожидаем, что результат вызова - `false`.
...
```TypeScript
  ...
  // Тестирование validateUser если пароль отличается
  ...
  // Тестирование validateUser нет пользователя
...
> jest
 ...
 PASS  src/users/users.service.spec.ts (6.59 s)
  User Service
    √ createUser (18 ms)
    √ validateUser - success (3 ms)
    √ validateUser - wrong password (2 ms)
    √ validateUser - wrong user
